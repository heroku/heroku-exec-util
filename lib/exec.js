...
function * initFeature(context, heroku, callback) {
  ...
  let promises = {
      ...
      app: heroku.request({path: `/apps/${context.app}`}),
      feature: heroku.request({ path: `/apps/${context.app}/features/runtime-heroku-exec` })
    }
  ...
  if (data.app['space'] != null && data.app['space']['shield'] === true) {
    cli.error(`This feature is restricted for Shield Spaces`);
