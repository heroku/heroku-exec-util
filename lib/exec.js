...
function * initFeature(context, heroku, callback) {
  ...
  if (buildpacks.length === 0) {
    ...
    if (addonUrl) {
      ...
      if (!feature.enabled) {
        yield cli.action('Initializing feature and restarting dynos', co(function* () {
        ...
        if (_hasExecBuildpack(buildpacks, buildpackUrl)) {
          yield cli.action(`Waiting for feature flag to propagate`, co(function * () {
            yield wait(3000)
          }))
...
function _execUrl(context, configVars) {
  ...
  if (urlString) {
    ...
    execUrl.auth = `${context.app}:${context.auth.password}`
    return execUrl
  }
}
...
function _hasExecBuildpack(buildpacks, url) {
  for (let b of buildpacks) {
    if (b['buildpack']['url'] === url) return true
  }
  return false
}
